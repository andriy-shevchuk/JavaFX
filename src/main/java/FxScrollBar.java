...
public class FxScrollBar extends Application{
    @Override
    public void start(Stage primaryStage) throws Exception {
        ...
        scrollX.valueProperty().addListener(event -> {
            rectangle.setTranslateX(scrollX.getValue()+20);
        });

        scrollY.valueProperty().addListener(event -> {
            rectangle.setTranslateY(scrollY.getValue()+20);
        });
        ...
        root.getChildren().setAll(scrollOpacity,scrollX,scrollY,rectangle);
